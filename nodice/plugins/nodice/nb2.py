...
async def _(bot: Bot, event: Event):
    ...
    if not isinstance(event, PrivateMessageEvent):
        group_id = event.group_id
    await bot.send(
        event, command_r(message, user_id=user_id, nickname=nickname, group_id=group_id)
    )
...
async def _(bot: Bot, event: Event):
    ...
    if isinstance(event, PrivateMessageEvent):
        ...
        if cal.roll_reason:
            await bot.send(
                event, format_string(getGlobalMsg("strRollDiceReason"), data)
            )
        else:
        ...
        if cal.roll_reason:
            await bot.call_api(
                "send_private_msg",
                user_id=event.sender.user_id,
                message=format_string(getGlobalMsg("strRollDiceReason"), data),
            )
        else:
            await bot.call_api(
                "send_private_msg",
                user_id=event.sender.user_id,
                message=format_string(getGlobalMsg("strRollDice"), data),
            )
